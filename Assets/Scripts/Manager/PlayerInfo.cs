...
{
    public static int?[,] deck_skill_ids { get; set; } = new int?[GameInfo.deck_count, GameInfo.max_skill_count];
    public static void SkillIdGetter(int deckNumber, out int?[] skillIds)
    {
    ...
    public static void SkillLevelGetter(int deckNumber, out int?[] skillLevels)
    {
    ...
    // スキルの数だけ手動で追加する必要がある
    public static Dictionary<int, bool> unlock = new Dictionary<int, bool>()
    {
    ...
    // スキルの数だけ手動で追加する必要がある
    public static Dictionary<int, int> level = new Dictionary<int, int>()
    {
